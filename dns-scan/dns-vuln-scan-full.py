...
if args_namespace.help or (not args_namespace.update \
    ...
    if len(rs_vul_list)==0:
        ...
        with open("DNS-Vulnerability-Report", "a") as report:
            ...
            print("\tCompleet kwetsbaarheidsrapport voor "+bcolors.OKBLUE+target+bcolors.ENDC+" genaamd "+bcolors.OKGREEN+"`DNS-Vulnerability-Report`"+bcolors.ENDC+" is beschikbaar onder dezelfde directory waar dns-scan zich bevindt.")
    ...
    for file_index, file_name in enumerate(tool_names):
        with open("DNS-Debug-ScanLog", "a") as report:
            try:
