	...
	<xsl:template match="/">
		<html lang="en">
			<head>
				...
				<title>
					Scan Report Nmap <xsl:value-of select="/nmaprun/@version"/>
				</title>
			</head>
			<body>
											...
											<tbody>
												<xsl:for-each select="/nmaprun/host">
													<xsl:for-each select="ports/port[state/@state='open']">
														<tr>
															<td>
																<a class="btn btn-info btn-sm" style="width: 100%;" data-toggle="modal">
																	<xsl:attribute name="data-target">
																		#Modal_<xsl:value-of select="translate(../../address/@addr, '.', '-')"/>
																	</xsl:attribute>
															...
															<td class="nobreak textcenter openPorts">
											...
											<div class="modal-dialog">
												<div class="modal-content">
													<div class="modal-header">
														...
														<h3 class="modal-title" style="text-shadow: 2px 2px Lightgray;">
															<xsl:attribute name="id">
																myModalLabel_<xsl:value-of select="translate(address/@addr, '.', '-')"/>
															</xsl:attribute>
															<xsl:value-of select="address/@addr"/>
														</h3>
													</div>
																			...
																			<xsl:for-each select="ports/port">
																				<xsl:choose>
																					<xsl:when test="state/@state = 'open'">
																						<tr class="success">
																							<td class="nobreak textcenter" title="Port">
																								<xsl:value-of select="@portid"/>
																							</td>
																							<td class="nobreak textcenter" title="Protocol">
																								<xsl:value-of select="@protocol"/>
																							</td>
																							<td class="nobreak textcenter" title="State / Reason">
																								<xsl:value-of select="state/@state"/>
																								<br/>
																								<xsl:value-of select="state/@reason"/>
																							</td>
																							<td class="nobreak textcenter" title="Service">
																								<xsl:value-of select="service/@name"/>
																							</td>
																							<td class="nobreak textcenter" title="Product">
																								<xsl:value-of select="service/@product"/>
																							</td>
																							<td class="nobreak textcenter" title="Version">
																								<xsl:value-of select="service/@version"/>
																							</td>
																							<td title="Extra Info">
																								<xsl:value-of select="service/@extrainfo"/>
																								...
																								<xsl:for-each select="script">
																									...
																									<ul style="padding-right:40px !important;">
																										<xsl:choose>
																																...
																																<xsl:for-each select="table">
																																	<tr>
																																		<td>
																																			<a target="_blank">
																																				<xsl:attribute name="href">
																																					https://vulners.com/<xsl:value-of select="elem[@key='type']"/>/<xsl:value-of select="elem[@key='id']"/>
																																				</xsl:attribute>
																																		...
																																		<xsl:choose>
																																			<xsl:when test="number(elem[@key='cvss']) >= 7">
																																				<td class="nobreak textcenter cvssh">
																																					<div class="bg-danger text-white font-weight-bold">
																																			...
																																			<xsl:when test="number(elem[@key='cvss']) >= 5">
																																				<td class="nobreak textcenter cvssm">
																																					<div class="bg-warning text-dark font-weight-bold">
																																						<xsl:value-of select="elem[@key='cvss']"/>
																																					</div>
																																				</td>
																																			</xsl:when>
																																			<xsl:otherwise>
																																				<td class="nobreak textcenter cvssl">
																																		...
																																		<xsl:choose>
																																			...
																																			<xsl:otherwise>
																																				<td class="nobreak textcenter">
																																					<div class="bg-secondary text-white font-weight-bold">
																																						<xsl:value-of select="elem[@key='is_exploit']"/>
																																					</div>
																																				</td>
																																			</xsl:otherwise>
																											...
																											<xsl:otherwise>
																												<pre style="white-space:pre-wrap; word-wrap:break-word; padding-right:40px !important;">
																													<code>
